...
Window_ActorCommand.prototype.create_com_name = function() {	
	this._com_name = new Sprite(new Bitmap(600,600))
...
Window_ActorCommand.prototype.refresh_com_name = function() {
	...
	this._com_name.opacity = 255;
	this._com_name.anchor.x = 0.5;
	this._com_name.anchor.y = 0.5;
    //ここにコマンド
	this._com_name.bitmap.drawText(this.command_name(),0,0,this._com_name.bitmap.width,32,"left");
    this._com_name.bitmap.drawText($gameVariables.value(this._actor.skills()[0].id + 200),90,420,this._com_name.bitmap.width,24,"center");
    if (this._actor.skills()[1]){
        this._com_name.bitmap.drawText($gameVariables.value(this._actor.skills()[1].id + 200),253,420,this._com_name.bitmap.width,24,"center");
    }
...
Window_ActorCommand.prototype.command_name = function() {
    ...
    if (this._index <= 1){
        return $dataSkills[this._index + 1].description;
    }else{
        return this._actor.skills()[this._index - 2].description;
    }  
...
Window_ActorCommand.prototype.update_battle_commands = function() {
	...
	for (var i = 0; i < com_length; i++) {
        //使えないやつをグレーアウト
        if (i == 2){
            if ($gameVariables.value(this._actor.skills()[0].id + 200) >= 1){
                this._com_sprites[i].setColorTone([0, 0, 0, 0])
            }else{
                this._com_sprites[i].setColorTone([0, 0, 0, 255])
            } 
        }else if(i == 3){
            if ($gameVariables.value(this._actor.skills()[1].id + 200) >= 1){
                this._com_sprites[i].setColorTone([0, 0, 0, 0])
            }else{
                this._com_sprites[i].setColorTone([0, 0, 0, 255])
            } 
...
Window_ActorCommand.prototype.update_position = function(i) {
	if (this._com_mode_b === 0) {
    	this._np[0] = 25 + this._wh[1] + (164 * i);//this._wh[0] + Moghunter.bcom_com_x + this._side[i][0];
	    this._np[1] = 100//this._wh[1] + (this._wh[1] * i) - this.limit_height() + Moghunter.bcom_com_y + this._side[i][1];
	} else {
